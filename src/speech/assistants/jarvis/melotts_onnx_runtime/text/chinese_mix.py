...
# Jieba writes normal dictionary initialization messages at INFO level. The TTS
# backend warms it in the background, so keep application startup logs focused on
...
def get_bert_feature(text, word2ph, device, model_path=None):
    from . import chinese_bert
    return chinese_bert.get_bert_feature(
        text, word2ph, model_id=model_path or model_id, device=device
    )
