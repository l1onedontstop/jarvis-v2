...
def distribute_phone(n_phone, n_word):
    ...
    for _ in range(n_phone):
        ...
        phones_per_word[phones_per_word.index(min_tasks)] += 1
...
def _predict_oov_without_nltk(word):
    """Spell an OOV tokenizer fragment using the bundled CMU dictionary."""
    ...
    if not normalized:
        return [word]
    phonemes = []
    for letter in normalized.upper():
        if letter == "'":
            continue
        pronunciations = eng_dict.get(letter)
        if pronunciations:
...
def g2p_old(text):
    ...
    for w in words:
        if w.upper() in eng_dict:
            ...
            phone_list = list(filter(lambda p: p != " ", _predict_oov_without_nltk(w)))
